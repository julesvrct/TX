...
#PREAMBULE
#---
#This script allows to build the euclidian classifier for bus lines
####################################

#install.packages("jsonlite")
library(jsonlite)
a<-read_json("data/lines.json", simplifyVector = TRUE)
b<-a[a$number=="5",] #Here we focus on bus line 5, but any line values can be used
...
source('functions/euclidian_classification.R')
...
y = euclidian_classification(dataligne,dataphone)
...
######  Classification sur tout les points de compiegne de classe VEHICULE  ######
source('functions/euclidian_classification.R')
