...
library(nnet)
c.nnet.perceptron.fit.random.tuning = caret::train(c.formule,
                                                   data = c.train.data,
                                                   method = "nnet",
                                                   trControl = c.fitControlRandomTuning,
                                                   tuneLength = 10
)
c.nnet.perceptron.fit.random.tuning
plot(c.nnet.perceptron.fit.random.tuning)


c.nnet.perceptron.grid = expand.grid(
  size = seq(1, min(8, round(mean(c(k, p)))), 1),
  decay = c(0.01, 0.1, 1, 10, 100)
)
c.nnet.perceptron.grid
c.nnet.perceptron.fit.grid.tuning = caret::train(c.formule,
                                                 data = c.train.data,
                                                 method = "nnet",
                                                 trControl = c.fitControl,
                                                 tuneGrid = c.nnet.perceptron.grid
)
c.nnet.perceptron.fit.grid.tuning
plot(c.nnet.perceptron.fit.grid.tuning)

c.nnet.perceptron.grid.size = expand.grid(
  size = seq(max(1, c.nnet.perceptron.fit.grid.tuning$bestTune$size-2), c.nnet.perceptron.fit.grid.tuning$bestTune$size+2, 1),
  decay = seq(max(0, c.nnet.perceptron.fit.grid.tuning$bestTune$decay-0.2), c.nnet.perceptron.fit.grid.tuning$bestTune$decay+0.2, 0.05)
)
c.nnet.perceptron.grid.size
c.nnet.perceptron.fit.grid.tuning.size = caret::train(c.formule,
                                                      data = c.train.data,
                                                      method = "nnet",
                                                      trControl = c.fitControl,
                                                      tuneGrid = c.nnet.perceptron.grid.size
)
c.nnet.perceptron.fit.grid.tuning.size
plot(c.nnet.perceptron.fit.grid.tuning.size)
...
c.nnet.multilayer.perceptron.grid = expand.grid(
  layer1 = 8,
  layer2 = 8,
  layer3 = 8,
  decay = seq(0,1,0.2)
)
c.nnet.multilayer.perceptron.grid
c.nnet.multilayer.perceptron.fit.grid.tuning = caret::train(c.formule,
                                                            data = c.train.data,
                                                            method = "mlpWeightDecayML",
                                                            trControl = c.fitControl,
                                                            tuneGrid = c.nnet.multilayer.perceptron.grid
)
c.nnet.multilayer.perceptron.fit.grid.tuning
plot(c.nnet.multilayer.perceptron.fit.grid.tuning)
...
c.nnet.one.hidden.final = c.nnet.perceptron.fit.grid.tuning
c.nnet.one.hidden.final$results$Accuracy[which(is.nan(c.nnet.one.hidden.final$results$Accuracy))] = 0
c.nnet.one.hidden.final.index = which(c.nnet.one.hidden.final$results$Accuracy == max(c.nnet.one.hidden.final$results$Accuracy))[1]
c.nnet.one.hidden.final.per.error = round((1-c.nnet.one.hidden.final$results$Accuracy[c.nnet.one.hidden.final.index])*100, 2)
c.nnet.one.hidden.final.per.error.ci = c(
  round((1-(c.nnet.one.hidden.final$results$Accuracy[c.nnet.one.hidden.final.index] + c.nnet.one.hidden.final$results$AccuracySD[c.nnet.one.hidden.final.index]))*100, 2),
  round((1-(c.nnet.one.hidden.final$results$Accuracy[c.nnet.one.hidden.final.index] - c.nnet.one.hidden.final$results$AccuracySD[c.nnet.one.hidden.final.index]))*100, 2)
)

c.nnet.three.hidden.final = c.nnet.multilayer.perceptron.fit.grid.tuning
c.nnet.three.hidden.final$results$Accuracy[which(is.nan(c.nnet.three.hidden.final$results$Accuracy))] = 0
c.nnet.three.hidden.final.index = which(c.nnet.three.hidden.final$results$Accuracy == max(c.nnet.three.hidden.final$results$Accuracy))[1]
c.nnet.three.hidden.final.per.error = round((1-c.nnet.three.hidden.final$results$Accuracy[c.nnet.three.hidden.final.index])*100, 2)
c.nnet.three.hidden.final.per.error.ci = c(
  round((1-(c.nnet.three.hidden.final$results$Accuracy[c.nnet.three.hidden.final.index] + c.nnet.three.hidden.final$results$AccuracySD[c.nnet.three.hidden.final.index]))*100, 2),
  round((1-(c.nnet.three.hidden.final$results$Accuracy[c.nnet.three.hidden.final.index] - c.nnet.three.hidden.final$results$AccuracySD[c.nnet.three.hidden.final.index]))*100, 2)
)
